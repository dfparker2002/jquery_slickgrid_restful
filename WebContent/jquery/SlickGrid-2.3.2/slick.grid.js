...
(function ($) {
  ...
  function SlickGrid(container, data, columns, options) {
    ...
    var plugins = [];
    var cellCssClasses = {};
    var cellCssClassesForModification = {};
    ...
    function _addClassToCell(addedHash) {
        var node, columnId, addedRowHash;
        for (var row in rowsCache) {
          addedRowHash = addedHash && addedHash[row];
    ...
    function _removeClassFromCell(removedHash) {
        var node, columnId, removedRowHash;
        for (var row in rowsCache) {
          removedRowHash = removedHash && removedHash[row];
    ...
    function addClassToCell(key, hash) {
        var prevHash = cellCssClassesForModification[key];

        cellCssClassesForModification[key] = hash;
        _addClassToCell(hash, prevHash);

        trigger(self.onCellCssStylesChanged, { "key": key, "hash": hash, "grid": self });
    }

    function removeClassFromCell(key, hash) {
        var prevHash = cellCssClassesForModification[key];

        cellCssClassesForModification[key] = hash;
        _removeClassFromCell(hash, prevHash);
    ...
    $.extend(this, {
      ...
      "removeClassFromCell": removeClassFromCell,
